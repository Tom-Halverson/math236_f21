...
a. Take the square roots of the *eigenvalues* of $A^T A$ to see that you get the singular values (the square root function is "vectorized" so you can do them all at once).
a. The matrix $V$ should be the eigenvectors of the matrix $A^T A$. Compute the eigenvectors of $A^T A$ to confirm this. **NOTE**: <span style="color:red"> The last eigenspace $E_0$ is two dimensional so there is not a single eigen-direction. There are many sets of orthgonal vectors for this two dimensional space and the svd command does not give the same ones as what you get here. Both are correct.</span>
