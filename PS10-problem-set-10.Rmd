...
\end{bmatrix}
$$
b. Compute the spectral decomposition $A = \lambda_1 \mathbf{u}_1 \mathbf{u}_1^T +  \lambda_2 \mathbf{u}_2 \mathbf{u}_2^T + \lambda_3 \mathbf{u}_3 \mathbf{u}_3^T+ \lambda_4 \mathbf{u}_4 \mathbf{u}_4^T$ and write the matrix $A$ as the sum of four rank-1 matrices. 
$$
\begin{bmatrix} 
1.6 & 0.7 & 1.4 & 0.3\\ 0.7 & 1.6 & 0.3 & 1.4 \\ 1.4 & 0.3 & 1.6 & 0.7 \\ 0.3 & 1.4 & 0.7 & 1.6 \\
\end{bmatrix}  =
...
Here is a $5 \times 6$ matrix.
```{r}
 (A = rbind(c(1 , 2 , 1 , 1 , 2 , 1 ),c(1 , 2 , 1 , 2 , -1 , 1 ), c(2 , 4 , 2 , 3 , 1 , 2 ), c(3 , -1 , 2 , -1 , -1 , 1 ), c(1 , 1 , 0 , 1 , 1 , 0 )))
```

a. Use the command `svd(A)` to command to compute the singular value decomposition of $A$. 
a. In the SVD we know that $A = U \Sigma V^T$. By multiplying by $U^T$ on the left and $V$ on the right we get  $U^T A V = \Sigma$. Extract the vectors $U$ and $V$ from the SVD from part (a) and multiply $U^T A V$ to get $\Sigma$.
